            ...
            <div class="max-w-md mx-auto">
                ...
                <div class="divide-y divide-gray-200">
                    <form action="{{ route('users.update', $user->getId()) }}" method="POST">
                        ...
                        @csrf
                        <x-forms.container>
                            <x-forms.input.text label="Name" name="name" :value="old('name', $user->name)" placeholder="Name">
                                @error('name')
                                    <x-forms.error :message="$message"/>
                                @enderror
                            </x-forms.input.text>
                            <x-forms.input.email label="Email" name="email" :value="old('email', $user->email)" placeholder="Email">
                                @error('email')
                                    <x-forms.error :message="$message"/>
                                @enderror
                            </x-forms.input.email>
